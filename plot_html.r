...
if(!dir.exists('data') | !file.exists('data/gapminder-berkeley-tidy.csv'))
{
  stop(paste('Error: can\'t find the tidied data! If it\'s been deleted from',
    'the repo, you can run tidy.r to recombine it (or just clone the repo',
    'again)'))
}
...
all_data = read_csv('data/gapminder-berkeley-tidy.csv')
...
stplot_ggplotly = ggplotly(stplot)
htmlwidgets::saveWidget(as.widget(stplot_ggplotly), 'stplot.html')
